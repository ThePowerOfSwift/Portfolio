...
class CameraViewController: UIViewController, AVCaptureFileOutputRecordingDelegate {
    ...
    @IBAction private func toggleCaptureMode(_ captureModeControl: UISegmentedControl) {
        if captureModeControl.selectedSegmentIndex == 0 {
            recordButton.isEnabled = false
        ...
        else if captureModeControl.selectedSegmentIndex == 1
        {
    ...
    func SnapShotFinished() {
        if isPhoto == true && isMovie == false {
            performSegue(withIdentifier: "sendFriendsVC", sender: PhotoData._photoData)
        }
    }
    ...
    func capture(_ captureOutput: AVCaptureFileOutput!, didFinishRecordingToOutputFileAt outputFileURL: URL!, fromConnections connections: [Any]!, error: Error!) {
        ...
        if success {
            ...
            isPhoto = false
            isMovie = true
            self.performSegue(withIdentifier: "sendFriendsVC", sender: outputFileURL!)
...
//            cleanup()
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "sendFriendsVC" {
            if let sendFriendsVC = segue.destination as? SendFriendsVC {
                
               
                if isMovie == true {
                    if let videoURL = sender as? URL {
                    sendFriendsVC.videoURL = videoURL
                    }
                }else{
                    if let snapData = sender as? Data {
                        sendFriendsVC.snapData = snapData
                    }
                }
