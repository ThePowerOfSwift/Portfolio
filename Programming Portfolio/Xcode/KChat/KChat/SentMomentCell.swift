...
class SentMomentCell: UITableViewCell {
    ...
    func configureCell(moment: Moment, image: UIImage? = nil){
        if moment._momentType == "Photo"{
            if image != nil {
                ...
                dateCreated.text = moment._dateCreated
                momentPreview.image = SentMomentsVC.sentImageCache.object(forKey: moment._previewImage as NSString)
                momentType.text = moment._momentType
                momentCaption.text = moment._caption
            }
        }
            ...
            if image == nil {
                if let imageUrl = NSURL(string:moment._previewImage) {
                    if let img = self.generateThumbnail(url: imageUrl) {
                        SentMomentsVC.sentImageCache.setObject(img, forKey: moment._previewImage as NSString)
                        DispatchQueue.main.async {
                            self.momentPreview.image = SentMomentsVC.sentImageCache.object(forKey: moment._previewImage as NSString)
